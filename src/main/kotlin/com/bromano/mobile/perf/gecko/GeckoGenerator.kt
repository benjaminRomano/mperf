...
private val libraryComparator =
    Comparator<Library> { libA, libB ->
        when {
...
object GeckoGenerator {
    fun getLibraryPathForSymbol(
        libraryList: List<Library>,
        symbol: SymbolEntry,
    ): String? {
        val dummyLib = Library("", "", symbol.address, "")
        val position = libraryList.binarySearch(dummyLib, libraryComparator)
    ...
    fun createGeckoProfile(
        ...
        val threads =
            samples
                .groupBy {
                    ...
                    val geckoSamples =
                        samples.sortedBy { it.sampleTime }.flatMap {
                            // Intern Frame
                            val frameIds =
                                it.backtrace.map { frame ->
                                    // Best effort try to find dsym string name
                                    val dsymFrame = frame.name

                                    // Intern String
                                    val stringId =
                                        stringMap.getOrPut(dsymFrame) {
                            ...
                            frameIds.reversed().forEach { frameId ->
                                prefixId =
                                    stackMap.getOrPut(Pair(prefixId, frameId)) {
        ...
        val traceStartTime = samples.minOfOrNull { it.sampleTime } ?: 0.0
    ...
    private fun getLibraryCategory(
        app: String?,
        frame: SymbolEntry,
        library: String?,
    ): Int {
