...
object InstrumentsParser {
     ...
     *   </row>
     */
    private fun createBinaryImageMapping(
        input: Path,
        runNum: Int = 1,
    ): Map<String, Library> {
    ...
    fun getInstrumentsSettings(
        input: Path,
        runNum: Int = 1,
    ): InstrumentsSettings {
        val runNode =
            queryXCTraceTOC(input)
        ...
        val timeProfileNodes =
            runElement
                .getElementsByTagName(TABLE_TAG)
                .asSequence()
    ...
    private fun getOriginalNode(
        ...
        val refId =
            node.getRefAttrValue()?.toLongOrNull()
    ...
    private fun Node.getFirstOriginalNodeByTag(
        ...
        getOptionalFirstOriginalNodeByTag(document, tag, originalNodeCache)
